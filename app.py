...
# Asset dello stile vettoriale OTM (sprite icone), scaricati da fetch_vector_style.sh
# in vector_style/. Lo sprite è referenziato dallo style come {base}/vector-style/otm_sprite.
app.mount("/vector-style", StaticFiles(directory="vector_style", check_dir=False), name="vector_style")

# Vector tiles for the mobile map (MapLibre). Generated by build_vector_tiles.sh
# into vector_tiles/area.mbtiles (OpenTopoMap-vector schema) and served per-tile
# from the mbtiles, so the app fetches them online and prefetches for offline —
# same shape as /map-tiles, but .pbf instead of .png.
_VECTOR_MBTILES = os.path.join(os.path.dirname(__file__), "vector_tiles", "area.mbtiles")


def _vector_db() -> sqlite3.Connection:
    if not os.path.exists(_VECTOR_MBTILES):
        raise HTTPException(status_code=503, detail="vector tiles not built")
    return sqlite3.connect(f"file:{_VECTOR_MBTILES}?mode=ro", uri=True)


@app.get("/vector-tiles/manifest.json")
def vector_manifest():
    con = _vector_db()
    try:
        rows = con.execute(
        ...
        con.close()
    # mbtiles usa lo schema TMS: riconverti la riga in y slippy/XYZ per il client.
    tiles = [[z, x, (1 << z) - 1 - r] for (z, x, r) in rows]
    zooms = [z for z, _, _ in tiles] or [0]
    return {
...
@app.get("/vector-tiles/{z}/{x}/{y}.pbf")
def vector_tile(z: int, x: int, y: int):
    tms_y = (1 << z) - 1 - y   # XYZ (slippy) -> TMS row usato da mbtiles
    con = _vector_db()
    try:
        row = con.execute(
...
# Stile MapLibre "OpenTopoMap vector" assemblato per l'app: i 63 layer OTM
# (vector/maplibregljs/otm_layers.json, scaricati da fetch_vector_style.sh) con le
# NOSTRE sorgenti. I layer di terreno (source dem / contour-source) sono scartati:
# OTM li genera via maplibre-contour, web-only e non disponibile su MapLibre RN —
# curve pre-cotte + hillshade nativo arriveranno come incremento a sé.
_VECTOR_STYLE_DIR = os.path.join(os.path.dirname(__file__), "vector_style")
_OTM_LAYERS = os.path.join(_VECTOR_STYLE_DIR, "otm_layers.json")
...
def vector_style(request: Request):
    ...
    return {
        "version": 8,
        "name": "GrappaSafe OpenTopoMap",
        "glyphs": _GLYPHS_URL,
        "sprite": f"{base}/vector-style/otm_sprite",
        "sources": {
            # Le nostre vector tile (schema OTM) del box del Grappa.
            "opentopomap-vector": {
                "type": "vector",
                "tiles": [f"{base}/vector-tiles/{{z}}/{{x}}/{{y}}.pbf"],
                "maxzoom": 14,
            },
            # Fallback raster fuori dal box (il layer raster è già negli otm_layers).
            "opentopomap-raster": {
                "type": "raster",
